...
import { Grid, Box, Typography, SxProps, Container, Button, FormControlLabel, Checkbox } from '@mui/material'
import CloseIcon from '@mui/icons-material/Close';
import CheckCircleIcon from '@mui/icons-material/CheckCircle';
import { getAllComp } from '~/api/competitionApi';
import { getAllCompUser } from '~/api/CompetitionUser';
import { InsertCompetitionBlog, getAllCompetitionBlog,EditCompetitionBlog,DeleteCompetitionBlog } from '~/api/CompetitionBlog';
import { getAllBlog } from '~/api/blogApi';
import useFetch from '~/hook/useFetch';
import { IBlog, ICompetition, ICompetitionBlogsUser, ICompetitionUser } from '~/interface/Interface';
...
const LinkCompetition = (props: PropLinkCompetition): JSX.Element => {
    ...
    const itemBlog: IBlog = allBlog?.payload?.find((r: IBlog) => r.blogId === props.blogId) || []

    const mergeListCompetition = allComps?.payload?.reduce((newList: ICompetition[], curr: ICompetition) => {
        const idCom = curr.comId;
        const check = allCompsBlogs?.payload?.find((r: ICompetitionBlogsUser) => r.comId === idCom)
        if (!check)
            newList.push(curr)
        return newList
    }, [])
    const listBlogNoLink = blogs.reduce((newList: number[], curr: IBlog) => {
        ...
        if (allCompsBlogs?.payload?.find((r: ICompetitionBlogsUser) => r.blogId === blogIdCurr) === undefined)
            newList.push(Number(blogIdCurr))
        return newList
    }, [])
    const itemBlogCompetitionHasLink:ICompetitionBlogsUser = allCompsBlogs?.payload?.find((r: ICompetitionBlogsUser) => r.blogId === props.blogId);
    ...
    const getCompetitionName=(comId:number):string=>{
        const comp:ICompetition = allComps?.payload?.find((r:ICompetition)=>r.comId === comId)
        return comp?.comName;
    }
    const handelAddCompBlogOK = (): void => {
        ...
        try {
            callInserCompetitionBlog(async () =>
                await InsertCompetitionBlog(requesData)
            )
    ...
    const handelEditCompBlogOK = (): void => {
        ...
        try {
            callEditCompetitionBlog(async () =>
                await EditCompetitionBlog(requesData)
            )
                                ...
                                <Grid container spacing={1}>
                                    <Grid item md={6} xs={12}>
                                        ...
                                        >
                                            ...
                                            >
                                                Tên blog: &nbsp;
                                                <span className='color-primary' style={{ fontWeight: "500" }}>
                                                    {itemBlog?.blogName}
                                                </span>
                                            </span>
                                            <Box
                                    ...
                                    >
                                        <Box>
                                            <h3 className='color-primary' style={{ textAlign: "center", marginTop: "0" }}>Các cuộc thi chưa liên kết</h3>
                                        </Box>
                                        {
                                            <Box
                                                sx={{
                                            ...
                                            >
                                                {
                                                    mergeListCompetition?.map((r: ICompetition, index: number) => (
                                    ...
                                    >
                                        ...
                                        </Box>
                                        {
                                            ...
                                            >
                                                ...
                                                >
                                                    <CheckCircleIcon/>{getCompetitionName(itemBlogCompetitionHasLink?.comId)}
                                                </Box>
                                                {
                                                    mergeListCompetition?.map((r: ICompetition, index: number) => (
                                                        competitionId === r.comId && checkCompetition ? (
